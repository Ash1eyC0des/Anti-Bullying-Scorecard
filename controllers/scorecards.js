...
module.exports = {
  ...
  getUserScorecards: async (req, res) => {
    ...
    try {
      const scorecards = await Scorecard.find({ userId: req.user.id }).populate(
        "school"
      );
  ...
  getEditScorecard: async (req, res) => {
    try {
      const scorecard = await Scorecard.findById(req.params.id).populate(
        "school"
      );
  ...
  editScorecard: async (req, res) => {
    try {
      ...
      await Scorecard.findByIdAndUpdate(req.params.id, {
        school: req.body.school,
        user: req.user.id,
        rating: req.body.rating,
        title: req.body.title,
      ...
      req.flash("success", {
        msg: "Your scorecard has been successfully updated.",
      });
  ...
  deleteScorecard: async (req, res) => {
    ...
    try {
      ...
      req.flash("primary", {
        msg: "Your scorecard has been successfully deleted.",
      });
    } catch (err) {
      console.log(err);
    }
